...
class MemoryAnalytics:
    ...
    def analyze_conversation_flow(
        ...
        for i in range(len(messages) - 1):
            current_role = messages[i].role.value if hasattr(messages[i].role, 'value') else messages[i].role
            next_role = messages[i + 1].role.value if hasattr(messages[i + 1].role, 'value') else messages[i + 1].role
