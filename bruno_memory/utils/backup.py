...
class BackupExporter:
            ...
            if msg_dict.get("metadata"):
                for field in fields_to_anonymize:
                    if field in msg_dict["metadata"]:
                        msg_dict["metadata"][field] = f"REDACTED_{hash(msg_dict['metadata'][field]) % 10000}"
    ...
    def create_backup_archive(
        ...
        if messages:
            self.export_messages_to_json(
                messages,
                str(archive_path / "messages.json")
            )
        
        if memories:
            self.export_memories_to_json(
                memories,
                str(archive_path / "memories.json")
            )
