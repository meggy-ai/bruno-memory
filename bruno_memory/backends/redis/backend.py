...
class RedisMemoryBackend(BaseMemoryBackend):
    ...
    async def retrieve_memories(
        ...
        try:
            ...
            for mem_id_bytes in memory_ids:
                ...
                if data:
                    ...
                    memory = MemoryEntry(
                        ...
                        conversation_id=UUID(memory_data['conversation_id']) if memory_data.get('conversation_id') else None,
            ...
            memories.sort(
                key=lambda m: m.metadata.importance if m.metadata else 0.0,
                reverse=True
            )
    ...
    async def search_memories(self, query: MemoryQuery) -> List[MemoryEntry]:
        ...
        try:
            ...
            if query.memory_types:
                type_values = [t.value if isinstance(t, MemoryType) else t for t in query.memory_types]
    ...
    async def delete_memory(self, memory_id: UUID) -> None:
        ...
        try:
            ...
            while True:
                cursor, keys = await self._client.scan(cursor, match=f"mem:*:{memory_id}", count=100)
