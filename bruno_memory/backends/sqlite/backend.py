...
from ...exceptions import (
    ConnectionError, StorageError, NotFoundError, ValidationError,
    DuplicateError, QueryError
)
...
class SQLiteMemoryBackend(BaseMemoryBackend):
    ...
    async def store_message(self, message: Message) -> None:
        ...
        try:
            ...
            """, (
                data['id'], data['role'], data['content'], data['message_type'],
                data['timestamp'], data['metadata'], data['parent_id'],
                data['conversation_id'], datetime.now(timezone.utc).isoformat()
            ))
    ...
    async def store_memory(self, memory_entry: MemoryEntry) -> None:
        ...
        try:
            ...
            """, (
                ...
                data['user_id'], data['conversation_id'], data['metadata'],
                data['created_at'], data['updated_at'], data['last_accessed'],
                data['expires_at']
            ))
    ...
    async def store_session_context(self, session: SessionContext) -> None:
        ...
        try:
            ...
            """, (
                data['session_id'], data['user_id'], data['conversation_id'],
                data['started_at'], data['ended_at'], data['last_activity'],
                data['is_active'], data['state'], data['metadata']
            ))
    ...
    async def get_context(self, user_id: str, conversation_id: str) -> ConversationContext:
        ...
        try:
            ...
            session_context = await self._get_latest_session_context(
    ...
    async def _store_user_context(self, user_context: UserContext) -> None:
        ...
        """, (
            data['user_id'], data['name'], data['preferences'],
            data['profile'], data['metadata'], data['created_at'], data['last_active']
        ))
    ...
    async def _get_latest_session_context(
        ...
        try:
            async with self._connection.execute("""
                ...
                LIMIT 1
            """, (user_id, conversation_id)) as cursor:
                row = await cursor.fetchone()
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self._connection.execute("SELECT COUNT(*) FROM conversation_contexts") as cursor:
                row = await cursor.fetchone()
                stats['total_conversations'] = row[0] if row else 0
            
            # Get active session count
            async with self._connection.execute("SELECT COUNT(*) FROM session_contexts WHERE is_active = 1") as cursor:
                row = await cursor.fetchone()
