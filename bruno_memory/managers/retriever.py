...
class MemoryRetriever:
    ...
    async def find_similar_memories(
        ...
        query = MemoryQuery(
            user_id=user_id,
            query_text=reference_memory.content,
            limit=limit
        )
    ...
    async def _hybrid_search_messages(
        ...
        fulltext_results = await self._fulltext_search_messages(
            query, conversation_id, limit
        )
        
        temporal_results = await self._temporal_search_messages(
    ...
    async def _exact_search_memories(self, query: MemoryQuery) -> List[MemoryEntry]:
        ...
        if query.query_text:
            memories = [
                m for m in memories
                if query.query_text.lower() in m.content.lower()
            ]
    ...
    def _apply_filters(
        ...
        if query.min_importance is not None:
            filtered = [
                m for m in filtered
                if m.metadata.importance >= query.min_importance
            ]
        ...
        if query.conversation_id:
            filtered = [
                m for m in filtered
                if m.conversation_id == query.conversation_id
            ]
    ...
    def _get_cached_results(self, cache_key: str) -> Optional[List[MemoryEntry]]:
