...
from bruno_core.models import (
    Message, SessionContext, ConversationContext, MessageRole
)

from ..base import BaseMemoryBackend
from ..exceptions import NotFoundError, ValidationError, StorageError
...
class ConversationManager:
    ...
    async def branch_conversation(
        ...
        new_session = await self.start_conversation(
            user_id=user_id,
            metadata={
                "branched_from": original_conversation_id,
                "branch_point": from_message_id
            }
        )
    ...
    async def get_active_sessions(self, user_id: Optional[str] = None) -> List[SessionContext]:
