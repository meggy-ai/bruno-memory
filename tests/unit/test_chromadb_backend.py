...
async def test_retrieve_messages(chromadb_backend):
    ...
    # Mock collection get
    chromadb_backend._collection.get = MagicMock(return_value={
...
async def test_search_similar(chromadb_backend):
    ...
    chromadb_backend._collection.query = MagicMock(return_value={
...
async def test_clear_all(chromadb_backend):
    ...
    chromadb_backend._client.get_or_create_collection = MagicMock(
...
async def test_connection_error():
    ...
    with patch('bruno_memory.backends.vector.chromadb_backend.chromadb.PersistentClient') as mock_client:
...
async def test_filter_by_min_score(chromadb_backend):
    ...
    chromadb_backend._collection.query = MagicMock(return_value={
        ...
        'metadatas': [[
            {'role': 'user', 'timestamp': datetime.now(timezone.utc).isoformat(), 'type': 'message'},
            {'role': 'user', 'timestamp': datetime.now(timezone.utc).isoformat(), 'type': 'message'},
            {'role': 'user', 'timestamp': datetime.now(timezone.utc).isoformat(), 'type': 'message'}
        ]],
